"""
Evaluation script for hierarchical RL: OPAL, HsO-VP, HILP.
"""
...
def run_single_skill_step(run_name,
               env, 
               agents_dict, 
               agents_log_dir, 
               log_video, 
    ...
    for actor_id, agent in agents_dict.items():
        ...
        agent.skill_inference_times = []
        agent.raw_action_inference_times = []
    ...
    while not done['__all__']:
        control_dict = {}
        for actor_id, agent in agents_dict.items():
            z = agent.run_high_step(obs[actor_id], timestamp)
            
            for i in range(len(z)):
                control_dict[actor_id] = agent.run_low_step(obs[actor_id], z, timestamp)
                
                next_obs, reward, done, info = env.step(control_dict)
                ...
                avg_inference_time = dict()
                for actor_id, agent in agents_dict.items():
                    avg_inference_time[actor_id] = sum(agent.raw_action_inference_times) / len(agent.raw_action_inference_times)
                
                obs = next_obs
        return list_render, ep_stat_dict, ep_event_dict, timestamp, avg_inference_time

@click.command()
@click.option("-c", "--config", default="eval_opal", type=str, help="Config to use")
def main(config: DictConfig):
    ...
    # start carla servers
    server_manager = CarlaServerManager(cfg.carla_sh_path, port=cfg.port)
    ...
    for ev_id, ev_cfg in cfg.actors.items():
        agent_names.append(ev_cfg.agent)    # ev_cfg.agent: opal
    ...
    for task_idx in range(ckpt_task_idx, env.num_tasks):
        ...
        list_render, ep_stat_dict, ep_event_dict, timestamp, avg_inference_time = run_single_skill_step(
            run_name, env, agents_dict, agents_log_dir, cfg.log_video)
        
        if cfg.wb:
            for actor_id, avg_time in avg_inference_time.items():
                wandb.log({f'time/avg_inference_time_{actor_id}': avg_time}, step=task_idx)

        # log video
        if cfg.log_video:
            video_path = (video_dir / f'{run_name}.mp4').as_posix()
            enc = ImageEncoder(video_path, list_render[0].shape, 30, 30)
            for im in list_render:
                enc.capture_frame(im)
            enc.close()
            enc = None
            if cfg.wb:
