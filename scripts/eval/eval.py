"""
Evaluation script for naive CQL, IQL.
"""
...
def run_single_step(run_name,
    ...
    for actor_id, agent in agents_dict.items():
        ...
        log.info(f"Agent {actor_id} model parameter count: {param_count}")
    ...
    while not done['__all__']:
        ...
        for actor_id, agent in agents_dict.items():
            avg_inference_time[actor_id] = sum(agent.inference_times) / len(agent.inference_times)

    return list_render, ep_stat_dict, ep_event_dict, timestamp, avg_inference_time

@click.command()
@click.option("-c", "--config", default="eval_iql", type=str, help="Config to use")
def main(config: DictConfig):
    ...
    for task_idx in range(ckpt_task_idx, env.num_tasks):
        ...
        list_render, ep_stat_dict, ep_event_dict, timestamp, avg_inference_time = run_single_step(
