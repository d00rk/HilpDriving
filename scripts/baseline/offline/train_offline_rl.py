...
def eval(q_function,
    ...
    with torch.no_grad():
        ...
        for i, (state, action, next_state, reward, terminal, _) in pbar:
            ...
            terminla = terminal.to(cfg.device, non_blocking=True)
            ...
            if cfg.algo == "iql":
                ...
                obs_expand = state.unsqueeze(1).expand(-1, cfg.num_random, C, H, W)
                obs_expand = obs_expand.reshape(B * cfg.num_random, -1)
    ...
    for epoch in range(cfg.num_epochs):
        ...
        for i, (state, action, next_state, reward, terminal, _) in progress_bar:
            ...
            if cfg.algo == "iql":
                ...
                obs_expand = state.unsqueeze(1).expand(-1, cfg.num_random, C, H, W)
                obs_expand = obs_expand.reshape(B * cfg.num_random, C, H, W)
                random_actions = random_actions.view(B * cfg.num_random, A)
