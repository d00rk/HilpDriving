...
from model.opal import Decoder
...
class OPALAgent:
    ...
    def setup(self, path_to_conf_file):
        ...
        # model
        # task policy: pi(z|s)
        self.task_policy = GaussianPolicy(cfg.model.state_dim, cfg.model.latent_dim)
        # decoder: p(a|z,s)
        self.primitive_policy = Decoder(cfg.model.state_dim, cfg.model.latent_dim, cfg.model.action_dim)
        
        # load checkpoint
        task_policy_ckpt = torch.load(cfg.model.task_policy_checkpoint_path)
        primitive_policy_ckpt = torch.load(cfg.model.primitive_policy_checkpoint_path)
        self.task_policy.load_state_dict(task_policy_ckpt['policy_state_dict'])
        self.primitive_policy.load_state_dict(primitive_policy_ckpt['decoder_state_dict'])
        
        self.task_policy.eval()
    ...
    def run_high_step(self, input_data, timestamp):
        ...
        with torch.no_grad():
            ...
            z = self.task_policy.act(birdview)
            end_time = time.time()
            self.skill_inference_times.append(end_time - start_time)
            self._logger.debug(f"Skill Inference time: {end_time - start_time}")
        return z
    ...
    def run_low_step(self, input_data, z, timestamp):
        ...
        with torch.no_grad():
            ...
            start_time = time.time()
            a_mu, a_logstd = self.primitive_policy(birdview, z)
            a_std = torch.exp(torch.clamp(a_logstd, min=-10, max=2))
            dist = torch.distributions.Normal(a_mu, a_std)
            action = torch.tanh(dist.sample())
            end_time = time.time()
            self.raw_action_inference_times.append(end_time - start_time)
            self._logger.debug(f"Raw Action Inference time: {end_time - start_time}")
